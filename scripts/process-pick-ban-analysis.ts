...
import { eq, sql, and, notInArray, isNull, ne } from "drizzle-orm";
...
async function processPickBanAnalysis() {
  ...
  const unresolvedVetoes = await db
    .select({
  ...
  if (unresolvedVetoes.length > 0) {
    console.error(`Cannot process pick/ban analysis with ${unresolvedVetoes.length} unresolved non-decider vetoes.`);
    for (const veto of unresolvedVetoes) {
      console.error(
        `match_id=${veto.matchId} veto_id=${veto.vetoId} order=${veto.orderIndex} action=${veto.action} map=${veto.mapName}`
      );
    }
    throw new Error("Unresolved null veto team_ids block pick/ban analysis.");
  }
  ...
  for (const match of allMatches) {
    ...
    if (!team1Id || !team2Id || !match.completed_at) {
      skippedMatches.push(`match_id=${match.id} missing team/completed_at`);
      continue;
    }
    ...
    if (vetoes.length === 0) {
      skippedMatches.push(`match_id=${match.id} has no vetoes`);
      continue;
    }
    ...
    for (let i = 0; i < vetoes.length; i++) {
      ...
      if (veto.action !== "pick" && veto.action !== "ban") {
        skippedMatches.push(`match_id=${match.id} has unsupported veto action ${veto.action}`);
        continue;
      }
  ...
  if (skippedMatches.length > 0) {
    console.error("Pick/ban analysis skipped invalid match data:");
    for (const skipped of skippedMatches) {
      console.error(`- ${skipped}`);
    }
    throw new Error(`Pick/ban analysis skipped ${skippedMatches.length} invalid match record(s).`);
  }
